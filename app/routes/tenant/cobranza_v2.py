        ...
        viviendas = (
            await ts.execute(
                text(
                    ...
                    f"  (SELECT cu.id FROM cuotas cu "
                    f"   WHERE cu.vivienda_id = v.id AND cu.estado != 'anulada' "
                    f"     AND cu.numero_recibo IS NOT NULL AND cu.numero_recibo <> '' "
                    f"   ORDER BY (cu.estado = 'pagado'), cu.periodo_anio DESC, cu.periodo_mes DESC "
                    f"   LIMIT 1) AS recibo_cuota_id "
                    f"FROM viviendas v "
